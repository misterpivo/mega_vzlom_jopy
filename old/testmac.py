...
# ----------------------
REQUEST_TIMEOUT_S = 15  # якщо модель не відповіла за стільки секунд — міняємо її

# таймаут на рівні SDK (в мілісекундах) — щоб обірвати "завислий" запит
client = genai.Client(
...
# моделі, що впали — пропускаємо до перезапуску програми
dead_models = set()
# прапорець "йде зміна моделі" — анімація показує стрілку
...
# АНІМАЦІЯ ОЧІКУВАННЯ (+ стрілка під час зміни моделі)
...
def show_switch_arrow(duration=0.6):
    """Показати стрілку в значку на час зміни моделі."""
...
# -------- виклик моделі з жорстким таймаутом 15с --------
def generate_with_timeout(model_name, timeout_s=REQUEST_TIMEOUT_S):
    holder = {}
    ...
    if t.is_alive():
        # потік ще працює — модель не вклалась у timeout_s
...
# -------- ШТУЧНИЙ ІНТЕЛЕКТ --------
def ask_gemini(question, fallback_attempts=2):
    history.append(f"User: {question}")

    raw = config.GEMINI_MODEL
    all_models = list(raw) if isinstance(raw, (list, tuple)) else [raw]

    last_error = None

    # --- РАУНД 1: по 1 спробі; будь-яка невдача (503/404/таймаут) -> у чорний список ---
    for model_name in all_models:
        if model_name in dead_models:
            continue
        try:
            answer = generate_with_timeout(model_name)
            history.append(f"Assistant: {answer}")
            return answer
        except errors.ClientError as e:
            last_error = e
            dead_models.add(model_name)
            print(f"⚠ {model_name}: помилка запиту ({getattr(e, 'code', '?')}) — у чорний список до перезапуску")
            show_switch_arrow()
        except (errors.ServerError, TimeoutError, Exception) as e:
            last_error = e
            dead_models.add(model_name)
            reason = "немає відповіді за 15с" if isinstance(e, TimeoutError) else f"({getattr(e, 'code', '?')})"
            print(f"⚠ {model_name}: {reason} — у чорний список до перезапуску")
            show_switch_arrow()

    # --- РАУНД 2: усі недоступні -> пробуємо ВСІ по {fallback_attempts} спроби ---
    ...
    for idx, model_name in enumerate(all_models):
        ...
        for attempt in range(fallback_attempts):
            try:
                answer = generate_with_timeout(model_name)
                history.append(f"Assistant: {answer}")
                dead_models.discard(model_name)  # ожила — повертаємо в гру
                return answer
            except errors.ClientError as e:
                # модель не існує / поганий запит — повтори марні
                last_error = e
                print(f"⚠ {model_name}: помилка запиту ({getattr(e, 'code', '?')})")
                break
            except (errors.ServerError, TimeoutError, Exception) as e:
                # 503/таймаут — чекаємо і пробуємо ще
                last_error = e
                wait = (2 ** attempt) + random.uniform(0, 1)
                code = "таймаут 15с" if isinstance(e, TimeoutError) else f"{getattr(e, 'code', '?')}"
                print(f"⏳ {model_name}: {code}, повтор через {wait:.1f}с...")
                time.sleep(wait)
